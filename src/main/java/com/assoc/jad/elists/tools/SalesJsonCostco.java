...
public class SalesJsonCostco implements SalesJson {
	...
	private void bldJsonList(String store) {
		BufferedReader br	= null;
		File costcoJson   = new File(COSTCOJSON);
		...
		try {
			...
			br.close();
			ShopListStatic.HashSales.put(store,jsonlist);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (br != null) try { br.close(); } catch (IOException e) {}
	...
	@SuppressWarnings("unchecked")
	public String getJsonObj(String store) {
		jsonlist = ShopListStatic.HashSales.get(store);
		if (jsonlist == null || jsonlist.size() == 0) bldJsonList(store);

		JSONObject jsonObj = new JSONObject();
		jsonObj.put(store,JSONArray.toJSONString(jsonlist));
